...
RESTART_FILE="$(cd "$(dirname "${RESTART_FILE}")" && printf '%s/%s\n' "$(pwd)" "$(basename "${RESTART_FILE}")")"
...
shopt -s nullglob
for candidate in "${OUTPUT_DIR}"/log.normal.restart[0-9]*.lammps; do
    filename="${candidate##*/}"
...
run_cmd cp "${RESTART_FILE}" "${OUTPUT_DIR}/$(basename "${RESTART_FILE}")"

LOCAL_RESTART="${OUTPUT_DIR}/$(basename "${RESTART_FILE}")"

run_cmd python "${SCRIPT_DIR}/prepare_lammps_normal_restart_input.py" \
