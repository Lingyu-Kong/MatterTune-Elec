    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        if steps > 0:
            handle.write(
                ...
                f"{dump_interval} {dump_name} id type x y z\n"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Continue normal MatterTune-MatterSim MLMD from a LAMMPS restart."
    )
    ...
    parser.add_argument("--steps-mode", choices=("additional", "total"), default="total")
    ...
    parser.add_argument("--restart-path-1", type=Path, required=True)
    parser.add_argument("--restart-path-2", type=Path, required=True)
...
def main() -> None:
    ...
    if not args.restart.is_file():
        raise FileNotFoundError(f"Restart file not found: {args.restart}")
    ...
    if not isinstance(pair_coeff_elements, list) or not all(isinstance(x, str) for x in pair_coeff_elements):
        raise ValueError(
            f"{args.source_metadata} does not contain a valid pair_coeff_elements list"
        )
    ...
    print(f"steps_mode={args.steps_mode}")
