...
def write_lammps_restart_input(
    *,
    ...
    with path.open("w", encoding="utf-8") as handle:
        ...
        # Save it before starting the continuation so the Python energy
        # segment can be converted from local step -> absolute step.
        ...
        if steps > 0:
            ...
            handle.write(
                f"dump            xtc_traj all xtc {xtc_dump_interval} {xtc_path}\n"
            )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a LAMMPS input that continues ghost-target FEP-TI from a restart file."
    )
    ...
    parser.add_argument("--restart-step-file", type=Path, required=True)
    ...
    parser.add_argument(
        ...
        default="total",
        help=(
            "total: run until the absolute production timestep reaches --steps; "
