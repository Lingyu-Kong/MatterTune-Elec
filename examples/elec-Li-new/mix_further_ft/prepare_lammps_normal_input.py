    ...
    *,
    ...
    with path.open("w", encoding="utf-8") as handle:
        handle.write(
            ...
            "normal MatterTune-MatterSim MD\n\n"
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare LAMMPS atomic data and input for normal "
            "MatterTune-MatterSim ML-IAP MD from an electrolyte PDB."
        )
    )
...
def main() -> None:
    ...
    requested_element_order = args.element_order
    ...
    payload = {
        ...
        "init_velocities": args.init_velocities,
        "restart_interval": args.restart_interval,
        "restart_path_1": str(args.restart_1),
        "restart_path_2": str(args.restart_2),
        "requested_element_order": requested_element_order,
        "present_elements": present_elements,
        "active_element_order": active_element_order,
        **metadata,
    }
    ...
    print(
        "present_elements="
        + ",".join(present_elements)
    )

    print(
        "active_element_order="
        + ",".join(active_element_order)
    )

    print(
        f"xtc_dump_interval="
        f"{args.xtc_dump_interval}"
    )

    print(
        f"xtc_path={args.xtc_path}"
    )
    ...
    print(
        f"restart_1={args.restart_1}"
    )

    print(
        f"restart_2={args.restart_2}"
    )
