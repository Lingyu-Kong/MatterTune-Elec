...
while [[ $# -gt 0 ]]; do
  case "$1" in
    --run-dir|--source-run-dir)
      SOURCE_RUN_DIR="$2"; shift 2 ;;
    --restart-file|--restart)
      RESTART_FILE="$2"; shift 2 ;;
    --output-dir)
      OUTPUT_DIR="$2"; shift 2 ;;
    --steps)
      STEPS="$2"; shift 2 ;;
    --steps-mode)
      STEPS_MODE="$2"; shift 2 ;;
    --total-steps)
      STEPS_MODE=total; STEPS="$2"; shift 2 ;;
    --additional-steps)
      STEPS_MODE=additional; STEPS="$2"; shift 2 ;;
...
merge_energy_segment() {
  ...
  if [[ ! -s "${RESTART_STEP_PATH}" ]]; then
    ...
    echo "Keeping segment for manual recovery:" >&2
    echo "  ${ENERGY_LOG_SEGMENT_PATH}" >&2
    return 1
  fi

  echo "Merging restart energy segment before exit..."

  python "${SCRIPT_DIR}/merge_lammps_fep_ti_energy_log.py" \
...
finalize() {
  ...
  if (( merge_status != 0 )); then
    echo "WARNING: energy merge failed with status ${merge_status}" >&2
  ...
  if (( status != 0 )); then
    echo "LAMMPS/restart workflow exited with status ${status}" >&2
  fi
