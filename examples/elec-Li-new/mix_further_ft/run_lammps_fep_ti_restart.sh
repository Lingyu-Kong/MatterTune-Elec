...
usage() {
    cat <<'EOF'
Usage:
  run_lammps_fep_ti_restart.sh --run-dir PATH [options]
...
Checkpoint priority:
  1. MATTERSIM_CKPT
  2. CKPT inherited from the current job
  3. checkpoint recorded in the old run config

All LAMMPS run-local files are interpreted relative to OUTPUT_DIR.
EOF
}
...
if [[ "${STEPS_MODE}" != "total" && "${STEPS_MODE}" != "additional" ]]; then
    echo "--steps-mode must be total or additional." >&2
    exit 2
fi
...
if [[ ! -f "${SOURCE_METADATA}" ]]; then
    echo "Missing source metadata: ${SOURCE_METADATA}" >&2
    exit 1
fi
...
if [[ -z "${RESTART_FILE}" || ! -f "${RESTART_FILE}" ]]; then
    echo "No usable restart file found: ${RESTART_FILE}" >&2
    exit 1
fi

RESTART_FILE="$(cd "$(dirname "${RESTART_FILE}")" && pwd)/$(basename "${RESTART_FILE}")"
...
if [[ -z "${CKPT}" ]]; then
    echo "No checkpoint was provided." >&2
    echo "Set CKPT or MATTERSIM_CKPT." >&2
    exit 1
fi

if [[ ! -f "${CKPT}" ]]; then
    echo "Checkpoint not found: ${CKPT}" >&2
    echo "Stored checkpoint from original run: ${STORED_CKPT}" >&2
    echo "Current CKPT=${CKPT}" >&2
    echo "You may override it with:" >&2
    echo "  export CKPT=/new/cluster/path/model.ckpt" >&2
    echo "or:" >&2
    echo "  export MATTERSIM_CKPT=/new/cluster/path/model.ckpt" >&2
    exit 1
fi
...
#
# The old config may contain /projects/... paths from another cluster.
# Only preserve the filename for run-local files.
#
...
#
# ----------------------------------------------------------------------
# Determine restart continuation number
# ----------------------------------------------------------------------
#
...
echo "CONTINUATION_TAG=${CONTINUATION_TAG}"
...
#
# ----------------------------------------------------------------------
# Generate portable restart input
# ----------------------------------------------------------------------
#
...
if [[ "${RUN_MD}" != "1" ]]; then
    echo "Prepared restart files only; RUN_MD=${RUN_MD}"
    exit 0
fi
...
# ----------------------------------------------------------------------
# IMPORTANT:
# Run LAMMPS from OUTPUT_DIR.
# Generated input only references run-local files by basename.
# ----------------------------------------------------------------------
#

if [[ "${DRY_RUN}" == "1" ]]; then
    echo "[dry-run] LAMMPS working directory: ${OUTPUT_DIR}"
...
#
# ----------------------------------------------------------------------
# Merge this restart segment into the accumulated FEP-TI log
# ----------------------------------------------------------------------
#
...
if [[ "${DRY_RUN}" != "1" && -f "${ENERGY_LOG_SEGMENT_PATH}" ]]; then
    run_cmd \
        ...
        --append-raw-segment "${ENERGY_LOG_SEGMENT_PATH}" \
        --temperature-log "${TEMPERATURE_LOG_PATH}" \
        --output "${ENERGY_LOG_PATH}"
fi

echo "Restart continuation completed."
echo "CONTINUATION_TAG=${CONTINUATION_TAG}"
echo "FINAL_DATA_PATH=${FINAL_DATA_PATH}"
