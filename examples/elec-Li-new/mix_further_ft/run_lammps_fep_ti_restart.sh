...
#
# Active restart FEP-TI segments are merged into the canonical energy
# files every two minutes.  This is intentionally not a user option.
#
...
if [[ "${STEPS_MODE}" != "total" &&
    ...
    echo \
        "--steps-mode must be total or additional: " \
        "${STEPS_MODE}" \
        >&2
...
if [[ ! -f "${SOURCE_METADATA}" ||
      ! -f "${SOURCE_CONFIG}" ]]; then

    echo \
        "Missing prepare_metadata.json or " \
        "run_lammps_fep_ti_config.txt under " \
        "${SOURCE_RUN_DIR}" \
        >&2
...
if [[ -z "${RESTART_FILE}" ||
    ...
    echo \
        "No usable restart file: ${RESTART_FILE}" \
        >&2
...
#
# ----------------------------------------------------------------------
# Current-cluster checkpoint takes priority over stale config path.
# ----------------------------------------------------------------------
#
...
#
# Old run configs may contain absolute paths from another cluster.
# Run-local output files are always rebuilt relative to OUTPUT_DIR.
#

stored_energy_log_path="$(
    config_value energy_log_path \
        || true
)"

stored_energy_log_raw_path="$(
    config_value energy_log_raw_path \
        || true
)"

stored_temperature_log_path="$(
    config_value temperature_log_path \
        || true
)"
...
if [[ -z "${CKPT}" ||
    ...
    if [[ -n "${STORED_CKPT}" ]]; then
        echo \
            "Original config checkpoint: ${STORED_CKPT}" \
            >&2
    fi
...
#
# ----------------------------------------------------------------------
# Recover orphan restart segments from earlier jobs.
#
# If a previous Slurm job was killed before its EXIT trap could execute,
# the hidden segment and restart-step files remain.  Recover them here
# automatically before starting another continuation.
# ----------------------------------------------------------------------
#
...
recover_orphan_segments() {
    ...
    while IFS= read -r segment; do
        ...
        if [[ ! -s "${restart_step_file}" ]]; then
            echo \
                "WARNING: orphan FEP-TI segment has no " \
                "restart-step file; keeping it:" \
                >&2
        ...
        echo \
            "[RECOVERY] merging orphan FEP-TI segment: " \
            "$(basename "${segment}")"
...
#
# ----------------------------------------------------------------------
# Determine the next continuation index.
# ----------------------------------------------------------------------
#
...
if [[ "${RUN_MD}" != "1" ]]; then
    echo \
        "Prepared restart files; skipping MD " \
        "because RUN_MD=${RUN_MD}."
...
#
# ----------------------------------------------------------------------
# Merge the currently active segment.
#
# keep_segment=1:
#     periodic snapshot while ML-IAP is still writing
#
# keep_segment=0:
#     final merge after MD exits; remove the segment
# ----------------------------------------------------------------------
#
...
merge_energy_segment() {
    ...
    if [[ ! -s "${RESTART_STEP_PATH}" ]]; then
        if [[ "${keep_segment}" != "1" ]]; then
            echo \
                "WARNING: energy segment exists but restart " \
            ...
            echo \
                "Keeping segment for automatic recovery:" \
                >&2
    ...
    while true; do
        ...
        if [[ -s "${ENERGY_LOG_SEGMENT_PATH}" &&
            ...
            merge_energy_segment 1 \
                || echo \
                    "WARNING: periodic restart FEP-TI " \
...
finalize() {
    ...
    if (( merge_status != 0 )); then
        echo \
            "WARNING: final energy merge failed " \
    ...
    if (( status != 0 )); then
        echo \
            "LAMMPS/restart workflow exited " \
            "with status ${status}" \
            >&2
    fi
